-- AleviChat production schema + RLS + Realtime
-- Supabase SQL Editor'da tek seferde calistirabilirsiniz.
...
create table if not exists public.messages (
  ...
  room text not null check (room in ('genel', 'oyun', 'sohbet')),
...
create index if not exists idx_dms_from_to_created_at on public.dms(from_user_id, to_user_id, created_at);
...
-- from_user_id eksik olan eski dms tablosunu duzelt
do $$
begin
  if exists (
    select 1
...
-- Auth user olusunca profile otomatik olustur
...
as $$
begin
  insert into public.profiles (id, email, username, city, about, role)
  values (
    new.id,
    coalesce(new.email, ''),
    left(split_part(coalesce(new.email, 'user'), '@', 1), 40),
    '',
    '',
    'user'
  )
